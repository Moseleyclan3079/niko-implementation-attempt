...
function item:onWorldUse()
    ...
    Game.world:startCutscene(function(cutscene)
        ...
        if lenParty then
            cutscene:textTagged("* (I could try to use my dark magic to open it...", "neutral", tag)
            cutscene:textTagged("* (I may break it tho.)", "neutral_b", tag)
            local c = cutscene:choicer({"Open it", "Please don't"})
            ...
            if success then
                ...
                cutscene:wait(0.5)
                cutscene:textTagged("* (...[wait:5]ops.)", "nervous", tag)
            end
        end
    end)
end
